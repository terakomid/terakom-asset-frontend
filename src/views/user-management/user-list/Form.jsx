...
import { Card, Grid, CardContent, Avatar, Stack, Typography, TextField, Box, MenuItem, Button, OutlinedInput, InputLabel, FormControl, InputAdornment, FormHelperText, IconButton, FormLabel, RadioGroup, FormControlLabel, Radio,  } from '@mui/material';
...
const Form = (props) => {
	...
	const onSubmit = (e) => {
		...
		if(props.title == "add"){
			...
			http.post(`user/${props.data.id}`, formData)
			.then(res => {
				// console.log(res.data)
				navigate('/user-list')
			})
					...
					<Card>
						<CardContent>
							<Stack direction="column" alignItems={'center'} >
								<input onChange={handleImage} type="file" name="image" style={{ display: 'none' }} id="image" accept="image/png, image/gif, image/jpeg, image/jpg" />
								<Box component="label" htmlFor="image">
									<Avatar sx={{ height: '20vh', width: '20vh', boxShadow: 1, mb: 2, cursor: 'pointer' }} alt="Remy Sharp" src={image.image_preview !== "" ? image.image_preview : DefaultImage} />						
								</Box>
								<Typography color="primary" variant="caption">Allowed *.jpeg, *.jpg, *.png, *.gif,  </Typography>
								...
								<Grid container spacing={3}>
									...
									<Grid item xs={12} md={6}> 
										<TextField 
											...
											disabled={!isComplete}
										>
											{departmentOptions.length > 0 && departmentOptions.map(v => (
												<MenuItem key={v.id} value={v.id}>{v.dept}</MenuItem>
											))}
